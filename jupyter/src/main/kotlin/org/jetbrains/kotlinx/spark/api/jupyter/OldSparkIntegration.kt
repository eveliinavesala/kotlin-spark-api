...
class OldSparkIntegration(
    ...
    override fun KotlinKernelHost.onLoaded() {
        ...
        properties {
            putIfAbsent(sparkMasterName, "local[*]")
            putIfAbsent(appNameName, "Kotlin Spark API - Jupyter")
            putIfAbsent("spark.sql.codegen.wholeStage", "false")
        ...
        val _1 =
            listOf(
                ...
                """.trimIndent(),
                """
                println("Spark session (Spark: $sparkVersion, Scala: $scalaCompatVersion, v: $version)  has been started and is running. No `withSpark { }` necessary, you can access `spark` and `sc` directly. To use Spark streaming, use `%use spark-streaming` instead.")
                """.trimIndent(),
                """
                ...
                """.trimIndent(),
                """
                fun <T> List<T>.toRDD(numSlices: Int = sc.defaultParallelism()): JavaRDD<T> = sc.toRDD(this, numSlices)
                """.trimIndent(),
                """
                fun <T> rddOf(vararg elements: T, numSlices: Int = sc.defaultParallelism()): JavaRDD<T> = sc.toRDD(elements.toList(), numSlices)
                """.trimIndent(),
                """
