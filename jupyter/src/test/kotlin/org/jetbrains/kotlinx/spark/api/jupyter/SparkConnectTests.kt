...
class SparkConnectTests :
    ShouldSpec({
            ...
            createRepl(
                ...
                eval {
                    librariesScanner.addLibrariesFromClassLoader(
                        ...
                        host = this,
                        notebook = notebook,
                        libraryOptions = mapOf(
                            "spark" to "3.5.1",
                            "scala" to "2.13",
                            // Make sure spark connect is running at sc://localhost!
                            "remote" to "sc://localhost",
                        ),
        ...
        context("Spark Connect") {

            withRepl {
                exec("%trackExecution")
                exec("sparkProperties.debug = true")

                should("Allow functions on local data classes") {
                    @Language("kts")
                    val klass = execEx("""@Sparkify data class Test(val a: Int, val b: String)""")
                    klass.shouldNotBeTypeOf<EvalResultEx.Error>()

                    @Language("kts")
                    val ds = execEx("""val ds = dsOf(Test(1, "hi"), Test(2, "something")).filter { it.a >= 1 }.showDS()""")
                    ds.shouldNotBeTypeOf<EvalResultEx.Error>()

                    @Language("kts")
                    val ds2 = execEx("""val ds = dsOf(Test(1, "hi"), Test(2, "something")).filter { it.a >= 1 }.showDS()""")
                    ds2.shouldNotBeTypeOf<EvalResultEx.Error>()

                    @Language("kts")
                    val filtered = execEx("""val filtered = ds.filter { it.a > 1 }""")
                    filtered.shouldNotBeTypeOf<EvalResultEx.Error>()

                    @Language("kts")
                    val filtered2 = execEx("""val filtered = ds.filter { it.a > 1 }""")
                    filtered2.shouldNotBeTypeOf<EvalResultEx.Error>()

                    @Language("kts")
                    val filteredShow = execEx("""filtered.showDS()""")
                    filteredShow.shouldNotBeTypeOf<EvalResultEx.Error>()

                    @Language("kts")
                    val filteredShow2 = execEx("""filtered.showDS()""")
                    filteredShow.shouldNotBeTypeOf<EvalResultEx.Error>()

                    @Language("kts")
                    val toList = execEx("""ds.toList<Test>()""")
                    toList.shouldNotBeTypeOf<EvalResultEx.Error>()

                    println(toList.toString())
                }

                should("Not work in the same cell") {
                    @Language("kts")
                    val klass =
                        execEx(
                            """
                            @Sparkify data class Test(val a: Int, val b: String)
                            val ds = dsOf(Test(1, "hi"), Test(2, "something"))
                            ds
                            """.trimIndent(),
                        )
                    klass.shouldBeInstanceOf<EvalResultEx.Error>()
                }
...
//            xshould("Have JavaSparkContext instance") {
//                @Language("kts")
//                val sc = exec("""sc""")
...
internal fun ReplForJupyter.execEx(code: Code): EvalResultEx = evalEx(EvalRequestData(code))

internal fun ReplForJupyter.exec(code: Code): Any? = (execEx(code) as? EvalResultEx.Success)?.renderedValue

@JvmName("execTyped")
internal inline fun <reified T : Any> ReplForJupyter.exec(code: Code): T {
    val res = exec(code)
    res.shouldBeInstanceOf<T>()
    return res
}

internal fun ReplForJupyter.execHtml(code: Code): String {
...
internal fun ReplForJupyter.execForDisplayText(code: Code): String {
    val res = exec<MimeTypedResult>(code)
